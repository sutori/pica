...
function R(rgba) { return rgba & 0xFF; }
function G(rgba) { return (rgba >>> 8) & 0xFF; }
function B(rgba) { return (rgba >>> 16) & 0xFF; }
function A(rgba) { return (rgba >>> 24) & 0xFF; }
...
function convolveHorizontally(src, dest, srcW, srcH, destW, filters) {
  ...
  for (srcY = 0; srcY < srcH; srcY++) {
    ...
    for (destX = 0; destX < destW; destX++) {
      ...
      srcPtr = (srcOffset + filterShift)|0;
      ...
      for (; filterSize > 0; filterSize--) {
        ...
        r = (r + filterVal * R(rgba))|0;
        g = (g + filterVal * G(rgba))|0;
        b = (b + filterVal * B(rgba))|0;
        a = (a + filterVal * A(rgba))|0;
      }
      ...
      destOffset = (destOffset + srcH)|0;
    }

    destOffset = (srcY + 1)|0;
    srcOffset  = ((srcY + 1) * srcW)|0;
  }
}
...
function resize(options) {
  var src32 = new Uint32Array(options.src.buffer);
  ...
  var tmp32 = new Uint32Array(destW * srcH);

  // To use single function we need src & tmp of the same type.
  // But src can be CanvasPixelArray, and tmp - Uint8Array. So, keep
  // vertical and horizontal passes separately to avoid deoptimization.

  convolveHorizontally(src32, tmp32, srcW, srcH, destW, filtersX);
  convolveVertically(tmp32, dest, srcH, destW, destH, filtersY);
